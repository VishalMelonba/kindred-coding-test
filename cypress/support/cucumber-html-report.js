...
const getOSName = () => {
    ...
    const osName = osNames[runInfo.osName];
    if (!osName) {
        console.log('Undefined or unsupported OS:', runInfo.osName);
...
const generateReport = () => {
    report.generate({
        ...
        metadata: {
            browser: {
                name: runInfo.browserName || 'unknown',
                version: runInfo.browserVersion || 'unknown'
            },
            platform: {
                name: getOSName(),
                version: runInfo.osVersion || 'unknown'
            }
        }
        ,
        customData: {
            ...
            data: [
...

                {
                    ...
                    value: dayjs(runInfo.startedTestsAt).format('YYYY-MM-DD HH:mm:ss.SSS') || 'unknown'
                },
                {
                    label: 'Execution End Time',
                    value: dayjs(runInfo.endedTestsAt).format('YYYY-MM-DD HH:mm:ss.SSS') || 'unknown'
                }
